<?php
defined('BASEPATH') OR exit('No direct script access allowed');

class Proyek extends CI_Controller {

    private $api_url = 'http://localhost:8080/api/proyek'; // URL API

    public function __construct() {
        parent::__construct();
        $this->load->library('curl');
        $this->load->helper('url');
    }

    public function index() {
        $data['proyek'] = json_decode($this->curl->simple_get($this->api_url), true);
        $this->load->view('proyek/index', $data);
    }

    public function tambah() {
        $this->load->view('proyek/tambah');
    }

    public function simpan() {
        // Mengambil data dari form input
        $data = array(
            'namaProyek' => $this->input->post('namaProyek'),
            'client' => $this->input->post('client'),
            'tglMulai' => $this->input->post('tglMulai'),
            'tglSelesai' => $this->input->post('tglSelesai'),
            'pimpinanProyek' => $this->input->post('pimpinanProyek'),
            'keterangan' => $this->input->post('keterangan'),
        );

        // Mengirimkan data POST ke API
        $response = $this->curl->simple_post($this->api_url, json_encode($data), array(CURLOPT_HTTPHEADER => array('Content-Type: application/json')));

        // Mengecek jika response kosong atau tidak
        if ($response) {
            redirect('proyek');
        } else {
            // Menampilkan error jika ada
            echo "Terjadi kesalahan saat menyimpan data.";
        }
    }

    public function edit($id) {
        $data['proyek'] = json_decode($this->curl->simple_get($this->api_url . '/' . $id), true);
        $this->load->view('proyek/edit', $data);
    }

    public function update($id) {
        $data = array(
            'namaProyek' => $this->input->post('namaProyek'),
            'client' => $this->input->post('client'),
            'tglMulai' => $this->input->post('tglMulai'),
            'tglSelesai' => $this->input->post('tglSelesai'),
            'pimpinanProyek' => $this->input->post('pimpinanProyek'),
            'keterangan' => $this->input->post('keterangan'),
        );

        $response = $this->curl->simple_put($this->api_url . '/' . $id, json_encode($data), array(CURLOPT_HTTPHEADER => array('Content-Type: application/json')));
        
        if ($response) {
            redirect('proyek');
        } else {
            echo "Terjadi kesalahan saat memperbarui data.";
        }
    }

    public function hapus($id) {
        $response = $this->curl->simple_delete($this->api_url . '/' . $id);
        
        if ($response) {
            redirect('proyek');
        } else {
            echo "Terjadi kesalahan saat menghapus data.";
        }
    }
}
